...
import { saveItem, catchItem } from "../../config/storage";
...
export async function addCube(conversation: MyConversation, ctx: MyContext): Promise<void>{
  ...
  if(modList.length === 0){
    ...
    ctx.reply(`Estes itens serão somados aos itens já existentes no seu inventário:\n\n${nonAdd.map((item) => ` - ${item.name} => ${ item.quantity}Un`).join("\n\n")}\n\nConfirma?`, { reply_markup: confirmAdd }); 
    
    }else if(nonAdd.length === 0){
      
    ctx.reply(`Estes itens serão adicionados:\n\n${modList.map((item) => `- ${item.name}: ${item.quantity}Un - ${item.weight}Kg => ${limitarCasasDecimais(item.weight * item.quantity, 3)}Kg\nDescrição: ${item.desc}`).join("\n\n")}\n\nPeso total a ser adicionado: ${modList.reduce((acc, item) => acc + limitarCasasDecimais(item.weight * item.quantity,3), 0)}Kg - Confirma?`, { reply_markup: confirmAdd });
      
    }else{
      ctx.reply(`Estes itens serão adicionados:\n\n${modList.map((item) => `- ${item.name}: ${item.quantity}Un - ${item.weight}Kg => ${limitarCasasDecimais(item.weight * item.quantity, 3)}Kg\nDescrição: ${item.desc}`).join("\n\n")}\n\nEstes itens serão somados aos itens já existentes no seu inventário:\n\n${nonAdd.map((item) => ` - ${item.name} => ${ item.quantity}Un`).join("\n\n")}\n\nPeso total a ser adicionado: ${limitarCasasDecimais(modList.reduce((acc, item) => acc + item.weight * item.quantity, 0) + nonAdd.reduce((acc, item) => acc + item.weight*item.quantity, 0),3)}Kg - Confirma?`, { reply_markup: confirmAdd });
